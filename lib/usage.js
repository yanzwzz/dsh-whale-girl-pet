 ...
 *
 * 【为什么优先用账本】
 *   账本的实时折叠覆盖**所有会话**（含子代理会话），并且带历史补扫
 *   （重启前 / 插件挂载前的用量）。而下面 `computeTodayUsage` 的回退路径只
 *   扫"当前在线会话"，重启后或子会话被回收后会偏小。所以只要账本可用就用它。
 *
 * @param ledger - {@link createUsageLedger} 的实例（可缺省）。
 * @returns 与 {@link computeTodayUsage} 同形的对象；账本不可用时返回 undefined。
 */
function todayUsageFromLedger(ledger) {
  try {
    if (ledger === null || typeof ledger !== 'object' || typeof ledger.snapshot !== 'function') return undefined;
    const snapshot = ledger.snapshot();
    const bucket = snapshot && snapshot.totals ? snapshot.totals.today : undefined;
    ...
    const tier = models.length > 0 && models.every((m) => tierOfModel(m) === 'flash') ? 'flash' : 'pro';
    const hit = Number(bucket.costHitCny) || 0;
    const miss = Number(bucket.costMissCny) || 0;
    const out = Number(bucket.costOutCny) || 0;
    const peak = Number(bucket.costPeakCny) || 0;
    const offPeak = Number(bucket.costOffPeakCny) || 0;
    const nowMs = Number(snapshot.now) || Date.now();
    return {
      ok: true,
      source: 'ledger',
      date: new Date(nowMs + bjOffset).toISOString().slice(0, 10),
      tokens: {
      ...
      costCny: round(hit + miss + out),
      costHitCny: round(hit),
      costMissCny: round(miss),
      costOutCny: round(out),
      costFlatCny: round(Math.max(0, hit + miss + out - peak - offPeak)),
      costPeakCny: round(peak),
      costOffCny: round(offPeak),
      tier,
...
/** 当前在线会话数（仅作展示用；取不到返回 undefined）。 */
function liveSessionCount(ctx) {
  try {
    const sessions = ctx === undefined || ctx === null || typeof ctx.get !== 'function' ? undefined : ctx.get('sessions');
    if (sessions === undefined || typeof sessions.list !== 'function') return undefined;
 ...
 * 汇总今日用量（按北京时间 0 点切分）。
 *
 * 优先读分时段账本：它覆盖所有会话（**含子代理会话**）且带历史补扫，
 * 所以重启或子会话被回收后依然算得准。账本不可用时回退到"遍历当前在线会话"
 * 的旧路径（`ctx.get('sessions').list()`，同样含子代理会话，只是少了历史）。
 *
 * @param ctx - 插件上下文。
 * @param ledger - {@link createUsageLedger} 的实例（可选）。
 * @returns 可 JSON 化的今日用量；服务不可用时返回 `{ ok: false, error }`。
 */
export function computeTodayUsage(ctx, ledger) {
  const fromLedger = todayUsageFromLedger(ledger);
  if (fromLedger !== undefined) {
    const count = liveSessionCount(ctx);
    if (count !== undefined) fromLedger.sessions = count;
    return fromLedger;
  }
  try {
