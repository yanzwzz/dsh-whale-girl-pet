...
/**
 * 单个会话的累计（token + 金额 + 调用次数）。
 *
 * 【为什么账本要额外记一份】
 *   DSH 的 `costUsage` 会话投影只折叠**当前会话自己的日志**，子代理（subagent）
 *   是独立会话，所以输入框下方的"会话费用"天然漏掉子会话。宿主侧的账本本来就
 *   在折叠**所有**会话的事件，顺手按会话 id 记一份累计，就能给费用 pill 提供
 *   "子会话花了多少"的权威数字（见 lib/index.js 的 /api/whale-pet/subtree-cost）。
 *
 * @returns 空的会话累计。
 */
function emptySessionCost() {
  return {
    input: 0,
    cacheRead: 0,
    cacheWrite: 0,
 ...
 * 把一份样本加进（或从）某会话的累计里。
 * @param target - 目标累计。
 * @param sample - {@link applySample} / {@link sampleNumbers} 的结果。
 * @param direction - 1 加入，-1 移除。
 */
function addToSessionCost(target, sample, direction) {
  target.input += direction * sample.input;
  target.cacheRead += direction * sample.cacheRead;
  target.cacheWrite += direction * sample.cacheWrite;
  target.output += direction * sample.output;
  target.hit += direction * sample.hit;
  target.miss += direction * sample.miss;
  target.out += direction * sample.out;
  target.peak += direction * sample.peak;
  target.offPeak += direction * sample.offPeak;
  target.calls += direction * 1;
}
...
export function createUsageLedger(options = {}) {
  ...
  /** 北京日序号 → 内部桶 */
  const days = new Map();
  /** occurrence key → { sample（纯数字）, hourIndex, dayIndex, sessionId }，插入序即时间序 */
  const occurrences = new Map();
  /** sessionId → 该会话的累计（子会话费用汇总用；与各时间桶同步加/减） */
  ...
  /** 取（或建）某会话的累计。 */
  function sessionCostOf(sessionId) {
    let cost = sessionCosts.get(sessionId);
    if (cost === undefined) {
      cost = emptySessionCost();
      sessionCosts.set(sessionId, cost);
    }
    return cost;
  }
  ...
  function ingest(sessionId, event) {
    ...
    if (previous !== undefined) {
      ...
      addToSessionCost(sessionCostOf(previous.sessionId), previous.sample, -1);
    ...
    if (occurrences.size > maxOccurrences) {
      ...
      if (!oldest.done) {
        ...
        addToSessionCost(sessionCostOf(stale.sessionId), stale.sample, -1);
  ...
  return {
    ...
    snapshot,
    /**
     * 某会话的累计（token + 金额 + 调用次数）。供宿主汇总"子会话花费"用
     * （DSH 的 costUsage 投影只折叠当前会话自己的日志，子代理是独立会话）。
     * @param sessionId - 会话 id。
     * @returns 累计的浅拷贝，或 undefined（该会话没有计费调用）。
     */
    sessionCost(sessionId) {
      const cost = sessionCosts.get(String(sessionId));
      return cost === undefined ? undefined : { ...cost };
    },
    /** 已计入过用量的全部会话 id（调用顺序）。 */
    sessionIds() {
      return [...sessionCosts.keys()];
    },
    /** 测试/诊断用：当前计入的调用数。 */
