...
function computeTodayUsage(ctx) {
  try {
    ...
    for (const session of sessions.list()) {
      // DSH ≥ 0.1.2-alpha.4 移除了 Session.events getter（perf: separate
      // indexed and snapshot log reads），改用 snapshotEvents()；兼容旧版
      // 保留 .events 回退，保证 0.1.2-alpha.3 及更早版本仍可用。
...
function computeTaskUsage(ctx, sinceMs) {
  try {
    ...
    let cost = 0;
    for (const session of sessions.list()) {
      // 同 computeTodayUsage：DSH ≥ 0.1.2-alpha.4 用 snapshotEvents()，旧版回退 .events
      const events = session && (typeof session.snapshotEvents === 'function' ? session.snapshotEvents() : session.events);
      if (!Array.isArray(events)) continue;
