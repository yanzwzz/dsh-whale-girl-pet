...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			'.dsh-cost-root{display:inline-flex;width:100%;max-width:var(--dsh-chat-content-width);margin:-20px auto 0;padding:0 calc(var(--dsh-composer-side-clearance) + 16px);box-sizing:border-box;justify-content:flex-end;font-size:var(--dsh-content-font-size-secondary,13px);line-height:calc(20px + var(--dsh-content-font-delta-secondary,0px));pointer-events:none}',
		...
		function useDashboardLayout(open, buttonRef, panelRef) {
			 ...
			 *
			 * 这里**不量面板当前尺寸**去落定：那会把当前渲染尺寸（CSS 自然宽
			 * 或上一次拖出来的尺寸）当成"复位目标"，导致复位后既不是默认尺寸、
			 * 又可能残留旧值。清掉记忆后直接 place()，走的就是默认尺寸。
			 */
			const clearLayout = () => {
