		...
		function Pet({ config }) {
			...
			const applyMood = (mood) => {
				if (mood === 'working') {
					...
					stopTyping();
					if (workingTimerRef.current) clearTimeout(workingTimerRef.current);
					// 收工三态：正在工作时被打断/结束 → **先播「工作结束」**（坐→站），
					// 播完由 handleEnded 自然进入随机链；不在工作中就直接回待机。
					if (wasBusy && !sleepRef.current && animRef.current !== WORK_END) {
						setAnim(WORK_END);
						setOnce(true);
						setSeq((s) => s + 1);
						return;
			...
			const playNotice = (ok, item) => {
				...
				// 已经在播这个动画就别重启：收工时 applyMood('idle') 可能刚切到「工作结束」，
				// 同批次的完成气泡只需要出气泡，不必让视频从头再播一遍。
				if (animName !== animRef.current) {
					setAnim(animName);
					setOnce(true);
					setSeq((s) => s + 1);
				}
			...
			useEffect(() => {
				...
				const poll = async () => {
					try {
						...
						// 权威工作状态自愈：宿主每次轮询都从 agent 注册表现算 running。
						// 状态转移原来是边沿事件驱动的（agent/status 只发变化），漏一条
						// mood 就会永远卡在"工作中"——手动停止后不回到待机就是这么来的。
						// 这里每个周期对一次账，事件丢了最多 800ms 也会被纠正。
						if (data && typeof data.running === 'boolean') {
							if (data.running) {
								if (!busyRef.current) applyMood('working');
								// busy 与动画脱钩（卡在待机链）时拉回工作轮播：
								// 只靠 busyRef 判定会漏掉"已经是 busy 但动画还在随机链"的
								// 状态，那时后续 mood:'working' 全是空操作。
								else if (animRef.current === IDLE) playWorking();
			...
			const handleEnded = () => {
				...
				// 【关键】工作中：任何"非工作链"动画播完都必须回到工作轮播。
				// 否则会掉进下面的随机链，而 busyRef.current 此时已经是 true ——
				// 后续 mood:'working' 会被 `if (!busyRef.current)` 挡掉，
				// 宠物就会**永远停在随机状态**（Agent 一直在工作，状态却不再变化）。
				// 多 Agent 交错、或 busy 与动画链脱钩（被打断的叫醒/通知动画）时最容易踩到。
				if (busyRef.current) {
					playWorking();
					return;
				}
